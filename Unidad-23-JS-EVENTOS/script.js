...
//Los data-attributes
//Son una forma muy simple de guardar datos en HTML

//Forma alternativa
//console.log(post_containerHTML.getAttribute('data-nombre'))

//Forma mas optima
//console.log(post_containerHTML.dataset.nombre)

function  renderPosts() {
    let template_HTML = ''
    for(const post of posts){
        let button_text = `Likear ${post.likes}`
        if(post.is_liked){
            button_text = `Quitar like ${post.likes + 1}`
        }
        template_HTML = template_HTML + `
        <div>
            <p>${post.content}</p>
            <button class='btn-like' data-post_id='${post.id}'>
                ${button_text}
            </button>
            <hr/>
        </div>
    ...
    //PRIMERO CREAMOS EL BOTON
    post_containerHTML.innerHTML = template_HTML

    //SEGUNDO LLAMAMOS A LA LISTA DE BOTONES DE LIKE POR CLASE
    //LLAMO A LA LISTA DE BOTONES QUE ESTA DENTRO DE EL CONTENEDOR
    const btn_like_list = post_containerHTML.getElementsByClassName('btn-like')
    for(const btn of btn_like_list){
        btn.addEventListener('click', togglePostLike)
    }
}   

function togglePostLike (event){

    //EVENT.TARGET es el elemento del HTML de donde se EMITIO el evento
    console.log(event.target)

    const button_clicked = event.target

    //ESTO ME PERMITE SABER A DONDE LE DOY CLICK, A QUE POST
    console.log(button_clicked.dataset.post_id)

    const post_id = button_clicked.dataset.post_id
    const post_selected = findPostById(Number(post_id))

    post_selected.is_liked = !post_selected.is_liked
    renderPosts()
}
